...
void testMCPReadOnlyContextTools() {
  std::istringstream in(
      ...
      "{\"jsonrpc\":\"2.0\",\"method\":\"tools/call\",\"params\":{\"name\":\"ccad_agent_methods\",\"arguments\":{}},\"id\":9}\n");
  std::ostringstream out;
  auto oldCin = std::cin.rdbuf(in.rdbuf());
  auto oldCout = std::cout.rdbuf(out.rdbuf());
  ...
  assertContains(out.str(), "ccad_agent_methods", "MCP lists agent methods tool");
  ...
  assertContains(out.str(), "schema_version", "MCP returns agent method catalog");
}
