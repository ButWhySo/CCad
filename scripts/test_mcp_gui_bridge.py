...
def main():
    ...
    child.stdin.write(json.dumps({"jsonrpc": "2.0", "id": 31, "method": "tools/call",
                                  "params": []}) + "\n")
    ...
    assert [message["id"] for message in messages] == [1, 2, 3, 30, None, 31, 4]
    assert messages[2]["error"]["code"] == -32601
    assert messages[3]["error"]["code"] == -32602
    assert messages[4]["error"]["code"] == -32602
    assert messages[5]["error"]["code"] == -32602
    tool_names = {tool["name"] for tool in messages[6]["result"]["tools"]}
