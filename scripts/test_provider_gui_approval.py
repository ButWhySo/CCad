"""No-network OpenAI-compatible provider to live CCad approval proof."""
...
class Stub(BaseHTTPRequestHandler):
    ...
    def do_POST(self):  # noqa: N802
        ...
        if "supervisor managing" in system:
            message, finish = {"role": "assistant", "content": "router"}, "stop"
        elif not Stub.tool_sent:
            Stub.tool_sent = True
            message, finish = {
            ...
            message, finish = {"role": "assistant", "content": "Applied."}, "stop"
        payload = json.dumps({"id": "ccad-gui-stub", "object": "chat.completion",
                              "created": 1, "model": request.get("model", "ccad-gui-stub"),
...
def main():
    ...
    try:
        ...
        if result.returncode:
            raise RuntimeError(output + f"stub_calls={Stub.calls}")
        assert '"pending_visible":true' in output
        assert '"via_count":1' in output
        assert '"received":true' in output
        assert Stub.calls >= 2, f"stub calls={Stub.calls}"
