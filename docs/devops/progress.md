...
**Current State**:
- **Sprint 986 (bounded project spatial and diagnostic retrieval)** is verified locally on `sprint-986-c3-complete-project-graph`. The typed index parses explicit bounding boxes, includes live DRC/ERC findings only through their actual linked objects within the requested area, searches diagnostic text, indexes all currently serialized PCB layer fields, and reuses unchanged project indexes when only UI selection/layer state changes. A 10k-track benchmark measures cold/cached/single-edit paths; cached retrieval improves from roughly 1.71 seconds to 0.22 seconds on this machine, while a one-track edit remains about 1.63 seconds because full snapshot extraction is still needed. Qt MinGW Release build passed (21/21), full CTest passed (113/113), and the provider-disabled GUI-map scenario completed seven mapped actions, verified the live `ZERO_LENGTH_TRACK` link to `T_SPRINT986_ZERO`, showed bounded context counts, and sent no provider request. Three distinct screenshots were inspected; stdout/stderr were reviewed. Staged secret scan and publication are in progress; no remote update is claimed yet.
