...
- Sprint: 1018
- Branch: `sprint-1018-passive-net-associations`
- Sprint 1018 extends C3 project retrieval with source-confirmed, same-sheet associations between explicitly declared `power_in` and `passive` pins that have unambiguous membership on the same exact schematic net. Retrieval works in either direction and from an unambiguously referenced PCB footprint; duplicate IDs/references, ambiguous memberships, wrong pin types, unconnected pins, and cross-sheet matches are suppressed. The relationship is not decoupling intent. The ContextBroker and provider-package path preserve relation provenance and count only entities surviving the package budget. Focused project-index contracts pass 55/55; Pyright reports zero diagnostics for both changed runtime modules; official Qt/MinGW Release build and full CTest pass 120/120. Workspace-only manifest `artifacts/evidence/sprint1018-source-passive-associations.json` (SHA-256 `9B9CB1046D4BB755310EEBA8247EDCCD8A37E14B9A39D6F947AAEE72FB1962B5`). No GUI behavior changed. Commit, local merge, push, and hosted-CI verification are pending.
- Sprint: 1017
