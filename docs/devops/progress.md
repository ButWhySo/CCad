...
**Current State**:
- **Sprint 978 (one Langfuse root per Agent turn)** is verified locally on `sprint-978-turn-trace-context` and is being prepared for publication. One durable thread-session-scoped root opens before context construction, with context assembly, automatic memory retrieval, package creation, and LangGraph invocation nested under it; roots close before flush/readback, including early provider-free exits. Automatic memory dedup compares bounded recent context and TurnRecords against the actual persisted thread recap, carries these exclusions into targeted refresh, and invalidates cache when recap content changes. Langfuse 4.7.1 with the OpenTelemetry in-memory exporter verifies shared trace IDs and parent relationships without network export. Qt MinGW Release build passes and full CTest passes 111/111. The isolated GUI-map run records seven mapped interactions, four inspected distinct screenshots, one real local context/memory turn without a provider request, and empty stderr. Pyright 1.1.414 is clean on telemetry, ContextBroker, and changed test contracts; it reports only that the large `orchestrator.py` is too complex to analyze. A remote Langfuse account trace is not claimed because this validation kept tracing disabled and used no credentials/network. Project-scoped memory/indexing, semantic retrieval, provider-tokenizer budgeting, preference/correction ranking, remote trace receipt, and trace continuity across approval resume remain open.
- **Sprint 978 publication**: pending commit and push to GitHub `main` after final CTest, secret scan, and diff review.
