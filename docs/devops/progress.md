...
- Sprint: 972
- Branch: `sprint-971-memory-store-activation`
...
**Current State**: 
- **Sprint 972 (provider SDK error fidelity)** is implemented on `sprint-972-provider-error-fidelity`. Offline contracts now distinguish structured quota/credit failures from transient rate limits, retain nested HTTP/gRPC status and bounded `Retry-After`, and prevent OpenAI/Anthropic SDK retry loops where supported. The pinned Gemini adapter still retries internally twice without a public setting; this is documented and tracked. Focused provider contracts pass, changed-module Pyright reports zero diagnostics, Qt MinGW Release build succeeds, and full CTest passes 104/104. The official mapped GUI run opened Agent Settings and confirmed provider identity plus masked-key rendering without sending a network request; every generated screenshot and captured stdout/stderr were inspected. Full per-provider live capability verification remains open.
