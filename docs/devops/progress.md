...
**Current State**:
- **Sprint 979 (Pyright analysis and protocol boundary)** is verified on `sprint-979-provider-catalog-modular`. Provider model-catalog HTTP clients and response parsing are isolated in `model_catalog.py`; the human-message turn handler is separated from the JSON-RPC loop without disabling the Pyright complexity guard. Targeted Pyright 1.1.414 reports zero diagnostics for `orchestrator.py` and `model_catalog.py`; Qt MinGW Release build and full CTest passed (112/112). The scoped provider-free GUI-map scenario completed seven mapped actions and verified chat persistence plus `/clear` projection behavior; all four distinct screenshots and stdout/stderr were inspected. Secret scan, commit, merge, and GitHub `main` publication are being completed in this turn.
