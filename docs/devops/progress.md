...
- Sprint: 1014
- Branch: `sprint-1014-ci-status-reconciliation`
- Sprint 1014 reconciles the reported CI/CT/CD failures against live evidence. The red hosted run `36101062608` belongs to old SHA `214df8f` and failed its Python protocol lane plus six Python-backed Linux CTests. Those failures were subsequently addressed by the Sprint 1010/1011 fixes already on `main`. Current `main` is `cae449782f841e443a534a5f7f4b1eef89d6c726`; hosted run `36239959391` passed all five configured jobs. Local Qt/MinGW Release build and CTest pass 120/120 with the required `C:\Qt\6.11.1\mingw_64\bin` runtime on `PATH`; omitting it produces Windows loader errors in GUI tests, not test assertion failures. Repository audit found no CD workflow or deployment target, so CD is unconfigured. No CI workflow change is required for this status correction. Workspace-only manifest `artifacts/evidence/sprint-1014-ci-status-reconciliation.json` (SHA-256 `24737FCD2FD9491610F41EB66DBEF4AFA68814959504C50F64B1D00011885033`).
