...
- Sprint: 1019
- Branch: `main` (Sprint 1019 merged; commit `8d678b92435bfa8a5ee0dad46f268f3273dae4de`)
- Sprint 1019 confirms hosted run `36256453720` is green on exact pushed `main` SHA `8d678b92435bfa8a5ee0dad46f268f3273dae4de`, with all five jobs passing and CTest included in the Linux core, Linux GUI, and Windows core jobs. Fifteen consecutive `main` runs passed; earlier red runs exposed already-repaired causes: missing Python test dependencies and Linux `-Werror` credential helper (Sprint 1010), then a test's dependency on an ignored local demo-board file (Sprint 1011). The official verifier rejected reuse of Sprint 1018's gate as stale, so a fresh Qt/MinGW Release build and full CTest were run: 120/120 passed. Manifest `artifacts/evidence/sprint1019-ci-ct-reconciliation-r2.json` (SHA-256 `C2937FE61E2F6867E98FC436D2D9A80F738600CED3EF47D43C1545ACBE9F369F`). There is one active workflow, `.github/workflows/ci.yml`; no CD workflow or deployment target is configured, so no CD failure exists to repair. The merged sprint branch was deleted locally and remotely. No source or GUI behavior changed.
- Sprint: 1018
- Branch: `main` (implementation branch `sprint-1018-passive-net-associations`)
- Sprint 1018 extends C3 project retrieval with source-confirmed, same-sheet associations between explicitly declared `power_in` and `passive` pins that have unambiguous membership on the same exact schematic net. Retrieval works in either direction and from an unambiguously referenced PCB footprint; duplicate IDs/references, ambiguous memberships, wrong pin types, unconnected pins, and cross-sheet matches are suppressed. The relationship is not decoupling intent. The ContextBroker and provider-package path preserve relation provenance and count only entities surviving the package budget. Focused project-index contracts pass 55/55; Pyright reports zero diagnostics for both changed runtime modules; official Qt/MinGW Release build and full CTest pass 120/120. Workspace-only manifest `artifacts/evidence/sprint1018-source-passive-associations.json` (SHA-256 `9B9CB1046D4BB755310EEBA8247EDCCD8A37E14B9A39D6F947AAEE72FB1962B5`). Implementation commit `fcf6847` was locally merged as `9ff6968bc11f21ddd45884bfe66223b75b87ac6a` and pushed; all five hosted CI jobs pass on that exact SHA in run `36253952081`. No GUI behavior changed.
- Sprint: 1017
- Branch: `main` (status-reconciliation branch `sprint-1017-ci-ct-cd-health`)
- Sprint 1017 reconciles the reported CI/CT/CD outage against live hosted and local evidence. The latest hosted run `36249116142` passes all five configured jobs on exact `main` SHA `011a754286d364b714dfbb64b268332d1b2c6a00`; preceding red runs were superseded by the existing Sprint 1010/1011 pipeline and clean-checkout CTest repairs. The official Qt/MinGW Release verifier passes all 120 local CTests when its required Qt runtime path is active; a raw invocation without that path fails only at Windows GUI DLL loading (`0xc0000135/0xc0000139`). The repository has one active CI workflow and no CD workflow or deployment target. Workspace-only manifest `artifacts/evidence/sprint1017-ci-ct-cd-health.json` (SHA-256 `BE9FC68EE72E446A7075AFE9D3D7A3693046C4E7500CF3E290BC9C0C0BE8E47C`).
- Sprint: 1016
- Branch: `main` (implementation branch `sprint-1016-project-pin-relationships`)
- Sprint 1016 C3 slice indexes embedded symbol-library definitions and declared pin metadata, links only exact unique component/pin identities across schematic and PCB models, and exposes safe fields through bounded context packaging. Focused contracts pass 49/49; changed-module Pyright reports zero diagnostics; Qt/MinGW Release build and full CTest pass 120/120. Workspace-only manifest `artifacts/evidence/sprint1016-project-pin-relationships.json` (SHA-256 `827A69DB649AD6DFFF0526877802B4DBE86CCE2D26847E477DBBC86355CF7C95`). Commit `5f1216a` was locally merged as `2ce38f9`, pushed, and all five hosted jobs passed for the exact pushed `main` SHA `2ce38f9a91fb1ad8590e4d112562e1d84f1a3dc4` in run `36248441890`. Standalone cached definitions, passive relationships, proposal/artifact links, and transaction-delta indexing remain open. This slice changes no GUI behavior.
- Sprint 1015 traced hosted failures #539/#540 to the ignored, untracked demo fixture read by project-index/context geometry contracts. The self-contained typed-fixture fix already on `main` (`4698996db768bd01f746a2a874edb07cff44a3c4`) passed all five jobs in #541; runs #542–#548 also passed all five configured jobs, including exact pushed `main` SHA `aa45f27ed1a78916a736d67acd2a6cec4d9a915f` in #548. The local verifier's build-reuse check was pinned to 115 tests while the current suite has 120; it now validates the complete zero-failure summary with any positive count and retains its source-timestamp guard. That timestamp guard conservatively rejects reuse after branch checkout refreshes file times even when Git source content is unchanged; content-bound freshness remains open. After that safe rejection, the official Qt/MinGW Release gate was rerun and full CTest passed 120/120. Workspace-only manifests: `artifacts/evidence/sprint-1015-ci-ct-pipeline-triage.json` (SHA-256 `B2D42AA4CC2B83F6306BC84A73BBB7F9DC8D6AA165B4DB6B617FA7A292442084`) and `artifacts/evidence/sprint-1015-ci-ct-hosted-closure-r1.json` (SHA-256 `17A22EB1EA936A12A030B2901E441292650A5B2BB8AA59642D72D8967AAC8479`). Commit `3536bc4` was locally merged as `ce8a518`; the final progress/checklist closure is `aa45f27`, pushed to `origin/main`, and all five hosted jobs passed in #548. CD remains unconfigured: no workflow or deployment target exists. Content-bound CTest evidence freshness remains the next open local-verifier follow-up.
- Sprint 1014 reconciles the reported CI/CT/CD failures against live evidence. The red hosted run `36101062608` belongs to old SHA `214df8f` and failed its Python protocol lane plus six Python-backed Linux CTests. Those failures were subsequently addressed by the Sprint 1010/1011 fixes already on `main`. Local Qt/MinGW Release build and CTest pass 120/120 with the required `C:\Qt\6.11.1\mingw_64\bin` runtime on `PATH`; omitting it produces Windows loader errors in GUI tests, not test assertion failures. Repository audit found no CD workflow or deployment target, so CD is unconfigured. The Sprint 1014 merge SHA `e22d07cec1c21876b7515a16555bb5c3cb3b8825` passed all five hosted jobs in run `36241720460`. Workspace-only verifier manifest `artifacts/evidence/sprint-1014-ci-status-reconciliation.json` (SHA-256 `24737FCD2FD9491610F41EB66DBEF4AFA68814959504C50F64B1D00011885033`).
- Sprint 1010/1011 reliability fixes remain green on `main`. Hosted CI run `36236321750` passed Agent Python, Linux core, Linux GUI, Windows core, and evidence-manifest for `dac80a9db546add56d110c464b4e1f31982791a4`. Sprint 1013's calculator implementation commit `cb2c184fc2202da4bbd976c0c91f814b9b4bce67` is merged and pushed to `main`; hosted run `36239405429` passed all five jobs (Agent Python, Linux core, Linux GUI, Windows core, evidence-manifest) for that exact SHA. CD is not a failing configured workflow: the repository has no deployment workflow or deployment target. Local Qt/MinGW Release and full CTest pass 120/120; focused calculator/catalog contracts and Pyright on the new calculator module pass. Workspace-only verifier manifest: `artifacts/evidence/sprint-1013-agent-calculator.json` (SHA-256 `2AD04B0909B363BEDB168A19AB9B1A75E6C2D305760E95DC8BC5266DE838059E`). No GUI behavior changed.
- Sprint: 1011
...
- Sprint 1008 is complete and published: `ContextBroker` supplies a bounded, deterministic initial Memory Summary containing eligible stable preference/correction records and explicitly high-importance facts, while excluding STM, low-importance facts, and secret-bearing rows. Focused contracts and changed-module Pyright pass; Qt/MinGW Release build and full CTest pass 119/119. The first gate exposed a CRLF-sensitive visual-harness source assertion; the fixture reader now normalizes Windows line endings and the full rerun passes. Workspace-only manifest `artifacts/evidence/sprint1008-memory-summary-r2.json` (SHA-256 `CEAC8CE780EC8DFDBB7E5B09FA494ECB29D4B07CC1DEAC98536C8D3D5A12433D`). Feature commit `a1603e0` was locally merged as `5d6aab6` and pushed; `git ls-remote origin refs/heads/main` confirmed remote main at `5d6aab66871cf93e389c215920bfc7ee33a5f099`. GitHub reports no check runs configured for this commit; local full gate is green.
- Sprint 1007 adds default-off Gemini exact CountTokens preflight using the bound adapter request, with safe exact/estimated context metadata and early categorized quota/auth stop; other providers and unsupported payloads remain estimates. Settings persistence/reload is verified through seven successful mapped clicks and ten inspected screenshots; offline contracts, Qt/MinGW Release, and full CTest pass 119/119. Workspace-only manifest `artifacts/evidence/sprint1007-gemini-exact-count-capturefix.json` (SHA-256 `A0D10CAD459E05FAA46FBC4DAA04200F75FB2BCD7AA73F5FB25E2ABE2733887A`). No live provider request or Langfuse receipt was attempted. Pyright found two existing orchestrator import-symbol diagnostics; clangd's bounded check was inconclusive. Feature commit `72bc954` was locally merged as `f47cb78` and pushed to `origin/main`; staged credential-pattern scan passed, with one pre-existing test-fixture-only token-shaped match at `scripts/test_memory_store.py:76`.
- Sprint 1006 allocates bounded project context from exact provider/model context limits learned only from an explicit model-catalog refresh. The registry is process-local, validated, bounded, and cleared for a provider after failed refresh; known windows allocate at most 25% and 8,192 estimated tokens, while unknown limits retain the configured character cap. Per-turn metadata reports source and estimation caveats; ordinary turns make no extra provider request. Focused contracts, changed-module Pyright, Qt/MinGW Release build, and full CTest pass; 117/117 tests. Workspace-only manifest `artifacts/evidence/sprint1006-model-context-allocation-r1.json` (SHA-256 `6754001F7D912E91F4E0F976EA78565A52B13CC17CB81CEEA1DE4C6BC4E6BC15`). Opt-in real Langfuse receipt, hosted CI/PR, secret scan, and integration remain open.
- Sprint 1005 instruments provider-returned token usage: bounded numeric counts are paired per generation with the approximate preflight estimate, written to Langfuse generation `usage_details`, aggregated across the active turn for final telemetry, and exposed in content-free opt-in development logs. Qt/MinGW Release and full CTest pass 116/116; the focused usage contract and changed-module syntax checks pass. `orchestrator.py` retains two known Pyright import-symbol diagnostics; the new accounting module is clean. Final-tree local manifest `artifacts/evidence/sprint1005-provider-usage-accounting-r2.json` (SHA-256 `0284C1508ED831158F8662FFEFF86BE1AD549B29817F25274A9972CCECE1009D`). Implementation is committed and pushed on the active feature branch; PR/hosted CI, exact pre-send tokenizer budgeting, and live remote Langfuse trace verification remain open.
- Sprint 1004 aligns the tracked visual-validation workflow with the root local instructions: every mapped interaction is recorded, while only distinct visual states are captured and every retained image is inspected. The replacement recipe was checked against the real verifier, app-owned GUI-map harness, commit hook, manifest checker, CI, and ignore rules; existing canonical infrastructure remains unchanged. The policy CTest passes, clangd reports zero errors, the evidence-manifest contracts pass 7/7, Qt/MinGW preflight succeeds, and full CTest passes 115/115. The first gate caught and recorded a policy-wording mismatch (114/115); the final CI-safe test handles the ignored local instructions file only when present. Final non-visual manifest `artifacts/evidence/sprint1004-visual-evidence-checkpoints-final2.json`, SHA-256 `F743B798128570479738A3983E9AA168C2571F02640DC0B9E319AD227A62E183`. Verify independent hosted CI against the exact pushed SHA before treating the branch as merge-ready.
- Sprint 1003 implements user-set memory importance 1–5 (neutral 3), compatible storage, safe RPC/command handling, bounded ranking after retrieval gates, and the Manage Memories UI. Focused contracts pass; changed-module Pyright has no new diagnostics beyond two pre-existing orchestrator import-symbol warnings, and clangd helped catch/fix a Settings type mismatch (`main.cpp` is clean; its optional Settings-refactor path hit a known clangd internal error). Qt/MinGW Release and CTest pass 115/115. The provider-disabled GUI-map flow persisted priority 5 with 15 successful mapped interactions; all 16 screenshots were inspected, stdout reviewed, stderr empty. Verifier manifest: `artifacts/evidence/sprint1003-memory-importance-release.json`, SHA-256 `981FF8D01D32234C15A16631839DE36D5CA7CA0FB345C60A5010C1D70E3E2E17`. Commit `b01ca30` is pushed to the feature branch; merge-to-main and hosted CI are unverified.
...
Sprint 1000 implements opt-in semantic retrieval over bounded typed PCB/schematic project descriptions. It reuses the existing ready local Ollama embedding backend, prioritizes BM25 hits under the 64-document embedding cap, caches document/query vectors only in process, preserves exact matches and relationship provenance, and carries safe status/count metadata through the provider context package. Raw track/via primitives are excluded. Focused contracts pass (47 project-index, 9 context-broker, plus memory and package contracts); changed retrieval modules report zero Pyright diagnostics. The final non-visual verifier passed Qt/MinGW preflight, Release build, and CTest 115/115. Manifest `artifacts/evidence/sprint1000-semantic-project-retrieval-final.json` SHA-256: `EEEF1B410719F7E3FB006986C49EFCBD6C983143147BCEECB64FD99C83BF73F4`. The Ollama service has no installed models, so no real-model retrieval benchmark was possible and none was downloaded. Generated entity summaries, persisted vectors, and full C4 remain open. Pyright still reports two existing `config` import-symbol diagnostics in `orchestrator.py`; the changed retrieval/package modules are clean.

Sprint 1001 adds explicit user-authored memory kinds (`fact`, `preference`, `correction`) throughout durable records, manager operations, slash commands, JSON-RPC, retrieval results, Manage Memories, and bounded provider context. Legacy rows load as facts without a rewrite; bounded deterministic weighting is applied only after relevance filtering, with content-free kind provenance. Focused contracts pass; Qt MinGW Release and full CTest pass 115/115. The isolated provider-disabled mapped GUI flow persisted exactly one typed preference, completed 12 mapped clicks plus three text entries, and all 15 screenshots were inspected; stdout confirms the mapped sequence and stderr is empty. Workspace-only manifest `artifacts/evidence/sprint1001-memory-kind-final.json`, SHA-256 `DF1A4209369829BCC600ED828A580E7BB1FEAEC46E70EC08CD8B87BAAAF1F0AB`. Pyright reports two existing `AgentConfigManager`/`ConfigPersistenceError` import-symbol diagnostics in `orchestrator.py`; no new changed-module diagnostics were introduced. The verified feature commit was pushed to `sprint-991-hybrid-memory-retrieval`; GitHub PR creation returned 403 and the hosted commit-status API returned no checks, so this branch was not merged to `main`. Provider-tokenizer budgets and recency/usage weighting remain open.

Sprint 998's Tier 1 functional-block net slice preserves typed group/sheet-to-native-net edges even when a net is already selected as an exact active-net seed. PCB and schematic net IDs remain namespaced, and the relationship does not claim physical or electrical connectivity. The live Agent context/workspace state reports only edges retained in the bounded package. The official Qt MinGW Release/full CTest/UI-map gate passed (115/115, 10 mapped interactions, four inspected screenshots, empty stderr); focused Python contracts passed and Pyright reports zero diagnostics for changed modules. Workspace-only manifest `artifacts/evidence/sprint998-functional-block-net-context-release.json` has SHA-256 `D154A62DCF6411FBD8933232F4DA43F290AC5596D78EE10AFEB5B9DFBF5DFFEA`. Commit `21bf3d6` is pushed to the feature branch; hosted CI inspection remains open pending a PR.

Sprint 999 fixes evidence-hook interoperability: PowerShell emits uppercase SHA-256 while the manifest-reference parser previously accepted lowercase only. A regression contract reproduces uppercase commit-message input; all seven evidence-checker tests pass, Pyright reports zero diagnostics, and the official non-visual Qt MinGW Release/full CTest gate passed 115/115. Manifest `artifacts/evidence/sprint999-evidence-hash-casing.json` has SHA-256 `91314D658A1BB7B683C7D9CBC370DFD111A6A1157636C00F868CA6A8708C8F72`; its preflight/build/CTest logs were inspected. Commit/push is included in this change set.

Sprint 997's Tier 1 slice is implemented: board-only nearby-footprint and exact placement-region retrieval preserve geometric relations through bounded context and Agent status, with inclusion/exclusion, coordinate separation, incremental refresh, and bounded-package contracts passing. Changed-module Pyright reports zero diagnostics; Qt MinGW Release build and full CTest pass 115/115. The final official mapped GUI gate passed 10 mapped interactions and four inspected screenshots, with empty stderr and no provider request. Workspace-only manifest `artifacts/evidence/sprint997-project-geometry-relations-ui-pass.json` has SHA-256 `104DC06FB7BA39509639D3BA7B49FE4D82BF5871AEC945E2B067C73C8B965C16`; captured images/logs remain local. Commit/push is confirmed at `21bf3d6`; hosted CI inspection remains open pending a PR.

**Current State**:
- **Sprint 999 (evidence digest casing)** fixes the mismatch between uppercase PowerShell `Get-FileHash` and lowercase-only manifest-reference parsing. Seven evidence-checker contracts and Pyright pass; Qt MinGW Release and full CTest pass 115/115. Manifest: `artifacts/evidence/sprint999-evidence-hash-casing.json` (SHA-256 `91314D658A1BB7B683C7D9CBC370DFD111A6A1157636C00F868CA6A8708C8F72`). Commit/push included in this change set.
- **Sprint 998 (explicit functional-block net context)** retains board-group and schematic-group/sheet native net edges through exact seed selection, bounded-package metadata, JSON-RPC, and Agent activity/workspace state. The active-net regression test failed before the fix and passes afterward. The provider-disabled GUI-map scenario verifies the exact retained count (`1`) on a disposable board and exercises the Settings dialog; Qt MinGW Release/full CTest passed 115/115, Pyright reports 0 diagnostics, and four scoped images were inspected with empty stderr. Manifest: `artifacts/evidence/sprint998-functional-block-net-context-release.json` (SHA-256 `D154A62DCF6411FBD8933232F4DA43F290AC5596D78EE10AFEB5B9DFBF5DFFEA`). Commit `21bf3d6` is pushed to the feature branch; hosted CI remains pending a PR.
- **Sprint 997 (PCB geometry relationships in bounded Agent context)** keeps board/schematic coordinates distinct, expands exact footprint and placement-region anchors into nearby/intersecting board objects, and prioritizes requested geometry over unrelated exact-match noise. Included relations/counts survive context packaging and IPC and are visible in Agent activity status. Focused project-index tests and full CTest pass (115/115); Pyright reports 0 diagnostics; the official mapped GUI run passed with 10 interactions and four inspected screenshots. Manifest: `artifacts/evidence/sprint997-project-geometry-relations-ui-pass.json` (SHA-256 `104DC06FB7BA39509639D3BA7B49FE4D82BF5871AEC945E2B067C73C8B965C16`). Commit/push confirmed at `21bf3d6`; hosted CI remains pending.
- **Sprint 996 (evidence workflow reconciliation)** compares the supplied replacement sketch with the repository's existing verifier, UI-map harness, evidence manifest checker, commit hook, CI lanes, and ignore rules. The existing implementations are retained; the workflow now documents their canonical paths, verified prior-gate reuse, and workspace-only screenshot/log handling. clangd reports zero diagnostics for the changed C++ policy contract, six evidence-checker contracts pass, and the Qt MinGW Release build plus full CTest pass 115/115. Official non-visual manifest `artifacts/evidence/sprint996-validation-reconciliation.json` has SHA-256 `06AD438359BB59D8D8B5D1B6E52A366CE9EBC2F2370CE9871268AD374382D45F`; screenshots are correctly not applicable. Redacted tracked-tree and staged-diff credential scans are complete; commit/push confirmed at `21bf3d6`, while hosted CI inspection remains pending a PR.
- **Sprint 995 (stable schematic pin identity persistence)** preserves non-empty native `SchPin.id` values through typed project JSON and leaves the field optional for legacy projects; retrieval provenance now distinguishes native IDs from derived IDs. Serializer and project-index regression tests pass, Pyright reports 0 diagnostics, Qt MinGW Release build passes, and full CTest passes 115/115. The official non-visual manifest is `artifacts/evidence/sprint995-schematic-pin-identities.json` (SHA-256 `21c857fa71413cdb262e3018cdd56db0485d46b6caae1383664b148e46254561`); logs stay workspace-only. clangd successfully loaded the project's Qt/MinGW compilation database and parsed the serializer AST, but stalled during indexing, so that analyzer result is explicitly incomplete. The verified source and docs are committed and pushed to the feature branch; hosted CI remains pending because no PR has been opened.
- **Sprint 994 (evidence-to-CI handoff)** clarifies the existing workflow: complete local verification and commit the passing manifest, push to trigger hosted CI, then merge only after checking the actual result for the pushed SHA. Screenshots and logs remain workspace-only per the active production TODO; the existing app-owned GUI-map harness, manifest checker, commit hook, and multi-platform CI are retained. Process-only changes now have an explicit `-NonVisual` verifier mode that still runs Qt preflight, Release build, and full CTest and records its not-applicable visual status; GUI changes still require a mapped interaction plan and inspected captures. The CTest policy guard and full 115/115 suite pass. Evidence manifest `artifacts/evidence/sprint994-process-gate.json` has SHA-256 `2d9a776277b623c81a95939c41aca1e89d6cfbe21e84ecca12eb73aa82bb0dc2`; build/test logs are workspace-only. This does not establish branch protection or claim a CI run.
- **Sprint 993 (explicit functional-block context)** is implemented and locally verified on `sprint-991-hybrid-memory-retrieval`. The revision-aware project index derives searchable blocks only from explicit typed PCB/schematic groups and serialized child-sheet membership, resolves bounded current typed members and net IDs, derives PCB bounds without mixing schematic coordinates, and carries provenance plus the active revision digest through bounded provider context. Agent context status reports the block count. Focused contracts pass 35/35; Pyright reports 0 diagnostics; clangd reports no source diagnostics (its optional extraction code-action warnings are not source errors); the Qt MinGW Release build and full CTest pass 115/115. The provider-disabled app-owned GUI-map flow completed 10 mapped interactions, inspected four meaningful screenshots, verified the isolated conversation database, and reviewed stdout/stderr (stderr empty); the live turn showed one functional block. Six evidence-manifest contracts pass, the modified PowerShell scripts parse, and `visual_harness_policy` CTest passes. Evidence: `artifacts/evidence/sprint993-functional-block-context-publish-ready.json` (SHA-256 `8753A1ABDA9444A08B8B1E41D5D3D34B129A107719D75E8903CB9AE552598400`). Logs/screenshots stay local; the commit hook verifies their hashes from the workspace, while CI can verify only committed manifest metadata. The first gate attempt found a harness-only thread-ID assertion mismatch; the expected isolated ID was corrected and the complete GUI/evidence stage passed against the unchanged application build. Semantic/vector retrieval remains explicitly unimplemented. Remote CI status must be checked after push.
- **Sprint 992 (declared schematic pins and annotation retrieval)** is implemented and locally verified on `sprint-991-hybrid-memory-retrieval`. The typed project index retrieves connected and unconnected symbol-declared pins, attaches only unambiguous net membership, indexes bounded schematic annotations/table text, excludes bitmap payloads, and removes deleted declarations incrementally. Focused project-index contracts pass 34/34; Pyright reports 0 diagnostics; Qt MinGW Release build succeeds; full CTest passes 115/115. The provider-disabled app-owned GUI-map flow completed 10 successful mapped interactions, inspected four meaningful screenshots, and reviewed stdout/stderr (stderr empty); the chat context showed three schematic pins and ten project matches, with no provider request. Evidence manifest: `artifacts/evidence/sprint992-schematic-pin-retrieval-verified-final.json` (SHA-256 `810853D3CF6C5509A4930D54CA2D74041A7832FF2A164D8EB245F773F7A54116`). The typed JSON model currently does not serialize `SchPin.id`, so file-loaded pin identities remain explicitly derived; broader C3 source-model graph completion remains open. Commit/push and remote CI are pending.
...
- **Sprint 987 (schematic metadata retrieval)** is verified and delivered to GitHub `main` from `sprint-987-c3-schematic-metadata`. Typed symbol field names/text/visibility and relative sheet title/path are indexed and returned through the bounded context allowlist; absolute sheet paths and sensitive property names are excluded. The Agent chat now sends the native project ID. Thirty-two focused contracts and changed-module Pyright pass; clangd reports no diagnostics for the changed Qt translation unit; Qt MinGW Release builds and CTest passes 113/113. The official provider-disabled UI-map scenario completed seven interactions, retrieved the ACME-42 field and relative Power Stage sheet path, retained transcript after `/clear`, and made no provider request. Three distinct screenshots were inspected and stdout/stderr reviewed. GitHub `main` was verified after push and the completed sprint branch removed; source-unsupported C3 items remain open.
- **Sprint 986 (bounded project spatial and diagnostic retrieval)** is verified and delivered to GitHub `main` as `5c69720`; remote SHA `5c6972085f70d10ca53de1dc03f4bef238441350` was confirmed. The typed index parses explicit bounding boxes, includes live DRC/ERC findings only through their actual linked objects within the requested area, searches diagnostic text, indexes all currently serialized PCB layer fields, and reuses unchanged project indexes when only UI selection/layer state changes. A 10k-track benchmark measures cold/cached/single-edit paths; cached retrieval improves from roughly 1.71 seconds to 0.21 seconds on this machine, while a one-track edit remains about 1.63 seconds because full snapshot extraction is still needed. Qt MinGW Release build passed (21/21), full CTest passed (113/113), and the provider-disabled GUI-map scenario completed seven mapped actions, verified the live `ZERO_LENGTH_TRACK` link to `T_SPRINT986_ZERO`, showed bounded context counts, and sent no provider request. Three distinct screenshots were inspected; stdout/stderr were reviewed. The staged secret-pattern scan was clean. The completed sprint branch was removed; broader C3 graph coverage remains open.
- **Sprint 985 (typed project relationships and live diagnostics)** is verified and delivered to GitHub `main` as `bf2b530`; remote SHA `bf2b530ca04024c2a3ad5373502631441f7565e5` was confirmed and the completed local sprint branch removed. Live `project.context` now carries bounded authoritative DRC/ERC diagnostics into typed retrieval, and explicit group membership, route endpoints, teardrop anchors, diagnostic targets, and schematic-page membership are retrievable without inferred links. Twenty-four focused project-index/context-broker tests pass; changed-module Pyright reports zero diagnostics; Qt MinGW Release and full CTest pass 113/113. The provider-disabled GUI-map run completed seven mapped interactions, confirmed `ZERO_LENGTH_TRACK` links to `T_SPRINT985_ZERO`, reported packaged diagnostic count, and inspected all nine screenshots plus stdout/stderr. The staged credential scan found no added credentials; whole-tree matches were historical artifact filenames, solder-mask option names, and synthetic test fixtures only. Broader C3 graph completeness remains open.
- **Sprint 984 (exact native PCB-net retrieval)** is verified and delivered to GitHub `main` as `06a6c3d`; remote SHA `06a6c3debb4495e7db6ce1ca55196c17bf3d5abf` was confirmed and the local sprint branch was removed. The project index derives exact, bounded `board_net` entries from IDs already carried by real typed board objects, connects those records to objects carrying the same ID, and explicitly avoids claiming geometric continuity. The safe context manifest and Agent activity line count only board-net entries that fit into the provider package. A structured pin identity outranks an embedded short net-name match. Focused contracts pass 23/23 and Pyright 1.1.414 reports zero diagnostics; Qt MinGW Release build and full CTest pass 113/113. The provider-disabled GUI-map query against `agent-catalog` completed seven mapped actions, visibly reported four packaged PCB-net records, verified transcript persistence and `/clear`, and made no provider request. All three distinct screenshots were inspected; stdout/stderr were reviewed. clangd 19.1.7 loaded the Qt/MinGW compile database but its token-action pass emitted internal ExtractFunction errors, so only the actual Qt compiler build and CTest are treated as authoritative C++ verification. The staged secret scan found zero credential-shaped additions; five historical whole-tree matches were false positives in `task-workspace` screenshot paths and `solder-mask` identifiers.
- **Sprint 983 (production-shaped project-index coverage)** is verified and delivered to GitHub `main` from `sprint-983-project-index-typed-geometry`. Regression coverage uses CCad's serialized nested `padstack.layer_set`, indexes typed board layer variants and route-request preferred layers, searches keepout/placement-region bounds, and retrieves scalar board design-rule values through compacted context. Focused contracts pass 20/20; Pyright 1.1.414 reports zero diagnostics for changed runtime modules; Qt MinGW Release builds and full CTest passes 113/113. The provider-disabled GUI-map flow completed seven mapped actions on a disposable board, reported 10 typed project matches and four PCB layer IDs, preserved the transcript across `/clear`, and made no provider request. Three feature-relevant screenshots were inspected; stdout/stderr were reviewed. The staged secret scan had zero findings; a repository-wide pattern hit was an expected synthetic token in the redaction test. GitHub's `main` SHA was verified after push, and the completed local sprint branch was removed.
- **Sprint 982 (multilayer typed-project context)** is verified, merged, and pushed to GitHub `main` as `322e8a2`; its local feature branch was removed. The typed index follows via start/end copper endpoints and declared pad layer sets, preserves included exact layer IDs through bounded context, and updates endpoint relationships incrementally. Normal canvas-click and automation via placement both use board-derived outer copper layers. Focused contracts pass 16/16, changed Python modules pass Pyright with zero diagnostics, Qt MinGW Release build and full CTest pass 113/113, and the official provider-disabled GUI-map flow completed nine mapped actions against a disposable project, proving saved F.Cu/B.Cu via endpoints and non-empty per-turn layer context. Five distinct screenshots were inspected; stdout/stderr were reviewed.
- **Sprint 981 (schematic net-member retrieval)** is verified on `sprint-981-schematic-project-graph`: bounded typed net members become retrievable `(net, component, pin)` entries tied to symbol identity, while physical board-net associations remain explicitly separate from electrical/geometric continuity. Incremental membership contracts prove stale member removal and unchanged-row preservation; extraction builds a bounded per-net lookup once. Pyright 1.1.414 reports 0 diagnostics; clangd 19.1.7 uses `build-qt-agent/compile_commands.json` and reports 0 errors for `main.cpp`; `agent_panel.cpp` has no compiler diagnostics but clangd's optional ExtractFunction token-action probe reports its known internal break/continue extraction errors. Qt MinGW Release builds and full CTest passes 113/113. The provider-disabled official GUI-map flow completed seven mapped actions, showed 10 project results including 3 schematic pins and 1 symbol, retained the canonical transcript after `/clear`, inspected all four generated images, and reviewed stdout/stderr.
- **Sprint 980 (typed project retrieval foundation)** is verified and merged/pushed to GitHub `main` as `4d87b23`; the completed local sprint branch was deleted. The implementation indexes the authoritative typed project snapshot for bounded exact/BM25, one-hop component/layer/shared-net, and coordinate/proximity retrieval; the broker reuses a revision-aware incremental index and the provider context retains relevant matches when a full snapshot is compacted. Focused retrieval/context/broker tests pass, Pyright reports 0 diagnostics, Qt MinGW Release builds, and full CTest passes 113/113. The app-owned UI-map flow completed seven mapped actions, showed 10 project matches for `U_DEMO` with provider disabled, confirmed durable transcript behavior, and passed after all four distinct screenshots and stdout/stderr were reviewed. `clangd` 19.1.7 reported zero errors for `main.cpp`; its `agent_panel.cpp` check did not finish within the validation window, while that file compiled successfully in the Qt build. Broader project graph and entity coverage remain open.
- **Sprint 979 (Pyright analysis and protocol boundary)** is verified and merged to GitHub `main` as `c367449`. Provider model-catalog HTTP clients and response shaping are isolated in `model_catalog.py`; the human-message turn handler is separated from the JSON-RPC loop without disabling Pyright's complexity guard. Targeted Pyright 1.1.414 reports zero diagnostics for `orchestrator.py` and `model_catalog.py`; Qt MinGW Release build and full CTest passed (112/112). The scoped provider-free GUI-map scenario completed seven mapped actions and verified chat persistence plus `/clear` projection behavior; all four distinct screenshots and stdout/stderr were inspected. The staged credential-pattern scan was clean, and the exact main commit was confirmed with `git ls-remote`.
- **Sprint 978 (one Langfuse root per Agent turn)** is verified and merged/pushed to `origin/main` as `1068ede`. One durable thread-session-scoped root opens before context construction; context assembly, automatic memory retrieval, package creation, and LangGraph invocation nest under it, and roots close before flush/readback, including early provider-free exits. Memory dedup includes the persisted recap and invalidates cache when recap changes. Langfuse 4.7.1 with the OpenTelemetry in-memory exporter verifies trace identity/ancestry without network export. Qt Release and CTest passed 111/111; the seven-action local GUI-map scenario produced four inspected distinct screenshots with reviewed logs. No remote Langfuse account trace is claimed. Project-scoped indexing, semantic retrieval, provider-tokenizer budgeting, preference/correction ranking, remote trace receipt, and approval-resume trace continuity remain open.
- **Sprint 977 (deterministic context and memory retrieval)** is verified and pushed to GitHub `main` as `3f5a246`. The new `ContextBroker` extracts bounded redacted signals, retrieves enabled task/thread/user memory before first model reasoning, caches by thread/revision/query/memory generation, exposes a bounded memory summary and content-free scope manifest in the provider envelope, and provides a real read-only local `ccad_search_memory` tool for targeted in-turn refresh. Pyright reports zero diagnostics on changed Python modules; clangd reports no diagnostics for the changed GUI translation unit; Qt MinGW Release build and full CTest pass (110/110). The isolated seven-action GUI-map scenario visibly attached one local memory record before reporting provider unavailability, then verified `/clear` projection behavior without sending a provider request; all four distinct screenshots, stdout, and stderr were inspected. CMake language-server initialization timed out, while actual CMake configure/build and CTest succeeded. Repository/staged secret scans found zero credential-pattern matches. Project-scoped memory, semantic/project indexing, same-root Langfuse `agent.turn` context nesting, tokenizer-exact accounting, and preference-aware ranking remain open.
- **Sprint 976 (durable conversation projection and TurnRecords)** is verified on `sprint-976-conversation-store-turnrecords`. SQLite stores per-thread messages and tool-call identity with recognized secret redaction; bounded model history is a separate projection, while `/cc` and `/clear` preserve canonical transcript. Each terminal turn produces a source-linked TurnRecord and bounded recap, with thread-local lexical retrieval added to provider context. Offline subprocess tests prove restart persistence and thread isolation with zero provider calls. The scoped seven-action GUI-map flow shows a real transcript turn, provider-unavailable response, `/clear` behavior, and inspected persisted SQLite counts; all four distinct screenshots were visually reviewed, stdout contained the expected completion record, and stderr was empty. Qt MinGW Release build and full CTest pass 108/108. Pyright 1.1.414 reports zero diagnostics; clangd 19.1.7 reports zero errors using `build-qt/compile_commands.json`; CMake 3.31.5 configure/generate/build validates target wiring. Existing checkpoint-only history migration, history UI/resume integration, and semantic retrieval remain open. Commit and GitHub publication follow the scoped diff/secret scan.
- **Sprint 975 (memory-management operation truth and lean visual evidence)** remains verified on `sprint-975-memory-ui-results`: Qt MinGW Release build succeeded; full CTest passed 106/106; memory lifecycle/persistence contracts pass; Pyright reports 0 diagnostics; clangd finds no C++ diagnostics in the changed main/UI-map files. The mapped flow verifies empty-write rejection, add/update/delete, confirmed reset of one isolated record, truthful operation statuses, zero remaining tier counts, and clean settings restoration. It logs every interaction and retains ten distinct screenshots for the UI states and destructive confirmations. Stdout/stderr are empty and no provider request was sent. The GUI-map test sentinel no longer paints over the live menu bar.
- **Sprint 974 (reviewed durable-memory compaction)** is verified and pushed on `sprint-974-memory-compaction`: Qt MinGW Release build succeeded; full CTest passed 106/106; focused Python contracts and changed-module Pyright passed (0 diagnostics). The isolated Agent UI-map flow completed seven mapped interactions and all seven screenshots were inspected; memory fixture hash stayed unchanged and stdout/stderr were empty. The plan/send/review/apply/cancel pipeline is implemented, but no real provider request was sent; live summary/apply remains opt-in. The commit and staged-diff secret scan are clean; repository scan found one pre-existing synthetic token-shaped test fixture and no real credential.
- **Sprint 973 (provider/model capability and limit truth)** is verified locally on `sprint-973-provider-compatibility`. Official provider documentation exposed a mismatch between the documented OpenRouter default and the actual `openrouter/free` runtime default, plus ambiguous Google `RESOURCE_EXHAUSTED` semantics. OpenRouter supported parameters and allowlisted Cerebras tool/context capabilities now survive catalog parsing without raw response payloads; stale unused Cerebras snapshot fields were removed; ambiguous Google limits use distinct quota/rate categories and are not retried. Provider contracts pass, changed-module Pyright reports zero diagnostics, Qt MinGW Release builds, full CTest passes 104/104, and the mapped local-only Settings flow passes with 18 inspected screenshots and clean stderr. Secret scan and commit/push remain pending; live model/tool capability has not been inferred from adapter tests.
- **Sprint 972 (provider SDK error fidelity)** is implemented on `sprint-972-provider-error-fidelity`. Offline contracts distinguish structured quota/credit failures from transient rate limits, retain nested HTTP/gRPC status and bounded `Retry-After`, and prevent OpenAI/Anthropic SDK retry loops where supported. Settings now explains exhausted quota/credits and displays a safe retry delay for explicit live-probe failures. The mapped GUI proves local-only validation returns `ready (network not probed)` and closes cleanly without contacting a provider. Focused provider contracts pass, changed-module Pyright reports zero diagnostics, Qt MinGW Release build succeeds, and full CTest passes 104/104. All 18 scoped screenshots and captured stdout/stderr were inspected. The pinned Gemini adapter still retries internally twice without a public setting; per-provider live capability verification remains open.
- **Sprint 971 (durable memory activation and preference persistence)** is implemented and verified on `sprint-971-memory-store-activation`: Qt MinGW Release build succeeded, full CTest passed 104/104, the three focused memory persistence contracts passed, and changed-module Pyright reported zero diagnostics. The real offline orchestrator subprocess verified corrupt-store fail-closed behavior, preference rollback, confirmed reset, disable-without-delete, checkpoint isolation, and project-file preservation. The mapped GUI flow created a durable LTM record, cancelled destructive reset, reopened Personalisation, matched all tier controls against initial and saved preferences, and cleanly closed. All 42 screenshots and stdout/stderr were inspected; the repository-wide secret-pattern review found only two old artifact-path false positives, and staged added lines had zero secret-pattern matches. Durable memory semantic compaction remains open.
- **Sprint 970 (semantic conversation-history compaction)** is implemented and locally verified on `sprint-970-semantic-history-compaction`: Qt MinGW Release build succeeded, CTest passed 103/103, offline semantic/checkpoint contracts passed, and changed-module Pyright reported 0 diagnostics. The official seven-action GUI-map scenario and all ten screenshots were inspected; it proved the no-history response sends no provider request, with empty stderr. The broad bundled Python contract sweep remains incomplete because unrelated source contracts failed and the independent local OpenAI-compatible boundary test hung; no provider-backed compaction or Langfuse readback is claimed. Secret scan, final publication, and traced compaction verification remain pending; durable memory-record semantic compaction stays open.
- **Sprint 969 (local large-context preview)** passed implementation and validation on `sprint-947-agent-tool-context` and is published with this commit. `/context [draft]` runs real request/memory/tool-schema accounting without provider invocation or history mutation. Runtime tests prove source text is absent; Qt MinGW Release build succeeds; CTest passes 100/100; Pyright reports zero diagnostics. The official GUI-map scenario completed eight mapped interactions; all nine screenshots and stdout/stderr were inspected. The diagnostic identifies `openrouter/openrouter/free`, counts 59 actual tool schemas, estimates 12,142 tokens, and reports the selected project's 19,862-character snapshot with `provider_request_sent:false`. clangd source diagnostics remain unverified because `--check` produced no result within 40 seconds.
- **Sprint 968 (task-scoped STM and duplicate safety)** is verified locally on `sprint-947-agent-tool-context`: Qt MinGW Release build succeeded, full CTest passed 99/99, and the official GUI-map flow completed eight mapped interactions with all screenshots/logs inspected, including a >20-second run. Pyright reports zero diagnostics for changed Python modules; staged secret scan, commit, and push are complete.
- **Sprint 967 (provider request and memory lifecycle)** is verified locally on `sprint-947-agent-tool-context`. Bounded context reporting and the shared three-tier memory manager are implemented, including explicit capture, scope/ranking, expiry, secret rejection, reset/delete, exact deduplication, and retention. Python tests pass and targeted Pyright is clean; Qt Release and CTest pass 98/98. The memory Settings/Manage UI-map pass completed with 26 inspected screenshots and clean logs. Sprint 969 provides provider-free large-context chat visual proof; provider-backed execution is intentionally not used for this local preview check.
...
## Sprint 475 progress update (2026-09-17)

Updated the agent capability registry so `RefillZones` resolves to `pcb refill-zones` as `first_slice`, with explicit `deterministic_contour_refill` caveat. Agent-serve regression passed; full Qt Ninja build completed 155/155; CTest passed 91/91; official harness passed, screenshot inspected, stderr empty.
...
### Sprint 527 - migrate add-pad loading

`pcb add-pad` now loads through `HeadlessBoardContext` while preserving pad validation, geometry, and audit-compatible writing. Full Qt build completed, CTest passed 91/91, and the official harness screenshot was inspected with empty stderr.
...
### Sprint 530 - migrate add-dimension loading

`pcb add-dimension` now loads through `HeadlessBoardContext` while preserving dimension validation, geometry, and audit-compatible writing. Full Qt build completed, CTest passed 91/91, and the official harness screenshot was inspected with empty stderr.
...
### Sprint 533 - migrate set-route-request loading

`pcb set-route-request` now loads through `HeadlessBoardContext` while preserving endpoint, layer, width, and update validation and audit-compatible writing. Full Qt build completed, CTest passed 91/91, and the official harness screenshot was inspected with empty stderr.
...
### Sprint 547 - migrate spread-footprints loading

`pcb spread-footprints` now loads the project through `HeadlessBoardContext` while preserving multi-component movement, gap handling, result reporting, and audit-compatible writing. Full Qt build completed, CTest passed 91/91 in 52.06 seconds, and the official harness screenshot was inspected with empty stderr.
...
The localhost provider roundtrip reached approval and via creation during investigation, but its embedded GUI-process test was nondeterministic before provider contact and is not in CI. The reliable provider-boundary test and offline mock approval test remain CI-safe; a deterministic app-owned provider fixture is the next task.
...
### Sprint 682 - accept standard MCP initialization notification

The GUI MCP stdio bridge accepts `notifications/initialized` silently, matching the post-initialize handshake and preventing unsupported-method errors from contaminating client streams. Offline bridge regression and official visual harness passed; screenshot was inspected and stderr was empty.
...
### Sprint 685 - classify unknown MCP tools correctly
...
### Sprint 698 - type into visible Settings controls
...
### Sprint 704 - make Agent Settings usable
...
Context-state protocol now reports opaque original size and a boolean
...
Sprint 752 fixes provider-run state truth: tool-call responses now report
...
Sprint 770 hardens all CI checkout steps with explicit clean working trees and
...
Sprint 848 publishes the provider-state response contract through the same
...
Sprint 850 corrects that contract to use the actual wire key `tool` (not
...
Sprint 838 makes `agent.list_models` capability metadata provider-specific:
OpenRouter is marked explicit network refresh, Cerebras none. This prevents
external harnesses from inferring network use from a shared top-level flag.
...
### Sprint 964 - Python control-catalog contract

Removed a duplicate `agent.set_config` descriptor and made memory runtime control metadata explicit: valid tier values, persisted/runtime effects, approval semantics, response fields, Python JSON-RPC transport, and non-callability as a model tool. Added dependency-free Python contract tests and registered them with CTest when a Python interpreter is available. Audited the kernel transaction API: it currently builds and serializes diffs/impact but has no apply/undo dispatcher, so no callable transaction tool was fabricated. Remaining work is to merge these control-plane descriptors into Qt discovery and to add transaction tools only alongside a real execution path. The Tier 2 registry item remains open.

Language-server checks: Pyright 1.1.414 reported zero diagnostics for the changed Python catalog and contract test. CMake language server 0.1.11 could not start because installed pygls 2.1.1 no longer exports the `LanguageServer` import it expects; CMake configure/generate succeeded for the changed CMake file. No C++ source changed in this slice, so clangd was not needed.

### Sprint 965 - Python controls in GUI discovery and themed focus

Merged validated Python JSON-RPC control descriptors into live GUI `agent.methods` and `agent.method_schema`, retaining `callable:false` and explicit Python transport/dispatch metadata. Added schema, uniqueness, secrecy, and preservation contracts. Suppressed Qt's dotted native focus primitive and provided themed focus/selection colors; board selection continues to highlight selected object geometry rather than drawing a generic focus border. Fixed an AgentRunner stop/start condition-variable race by synchronizing the running predicate with the queue mutex; the dedicated test passed 30 repeats. Global language-server validation now succeeds with clangd 19.1.7 using the Qt/MinGW compile database, Pyright 1.1.414 reporting zero diagnostics on the changed Python catalog, and cmake-language-server 0.1.11 completing JSON-RPC initialize with compatible pygls 1.3.1. clangd's Settings translation unit had no source diagnostics; its optional code-action self-test reports an internal invalid break/continue extraction case. Qt/MinGW Release build and full CTest passed 93/93. Live GUI-map validation exercised Agent Settings, Personalisation controls, object selection, and menus; screenshots were inspected and stdout/stderr were empty. Preferences-menu child actions remain unmapped and are still open in the TODO.

### Sprint 966 - provider failures and development Langfuse delivery

Provider errors are classified across chained/contextual exceptions and exception groups, preserving inner SDK status and quota/rate-limit categories. Chat failure text maps categories to actionable provider guidance without forwarding exception bodies, secrets, or asserting that no tool ran. In development, each completed or failed graph turn flushes export and fetches the exact trace ID from Langfuse before reporting `verified`; Settings and stderr include per-turn ID, spans, and result. A live GUI request returned `trace test`; readback verified the current trace with 16 spans. The focused contracts pass, Qt Release build succeeds, full CTest passes 93/93, and GUI screenshots plus empty stdout/stderr were inspected. Pyright has zero diagnostics in telemetry; six existing orchestrator typing diagnostics remain. The full provider/context/package accounting and memory-tier lifecycle report is still open.

### Sprint 968 - task-scoped STM and duplicate safety

STM now has an explicit task UUID scoped to one durable Agent session. `/task start`, `/task status`, and `/task end` run in the local command parser, without a model request; task replacement, end, or bounded-session eviction clears its process-only STM. Turns outside active task scope cannot retain STM. Exact normalized duplicate adds return the existing entry; add/update reject high lexical overlap and identify the conflicting ID, without claiming semantic similarity. Slash suggestions refresh on text changes from keyboard or UI-map input.

Qt MinGW Release build passed; full CTest passed 99/99. The app-owned GUI-map scenario performed eight mapped actions covering tab selection, slash suggestions, keyboard selection, and task start/status/end. All 12 screenshots from each of two runs were inspected, stdout/stderr were reviewed, and the extended run remained open over 20 seconds. Pyright reports zero diagnostics for changed Python runtime modules. This sprint was committed and pushed after its staged-diff secret scan.

### Sprint 969 - local large-context preview

Added `/context [draft]` to calculate and display a privacy-safe explanation using the same bounded context, memory retrieval, system prompt, conversation, and provider-bound tool schemas as a real request. The draft is used only for hypothetical memory retrieval and accounting; this local command does not invoke LangGraph/provider execution, execute tools, enter chat history, or display source content. The report includes estimated source token counts, memory tier state and ranking, omissions, model context-limit availability, and an explicit no-provider-request label. `/commands` and the slash palette expose the command. The official app-owned UI-map harness lowers the threshold only in its child GUI process to test the large branch and captures output for inspection. Runtime, static-language-server, screenshot/log, Qt Release, full CTest, secret-scan, commit, and push evidence will be recorded only after validation completes.

### Sprint 974 - reviewed durable-memory compaction

Added bounded durable LTM/episodic compaction with a local plan, explicit provider-send command, reviewable process-only summary, and separate atomic apply/cancel commands. Tools are disabled for summary generation. Stale records are refused before provider transmission and revalidated at commit; memory plans are discarded on tier disable or namespace change. Focused Python/JSON-RPC contracts, Pyright (0 diagnostics), the Qt MinGW Release build, and full CTest (106/106) pass. Seven mapped GUI actions were exercised in an isolated profile; all seven screenshots were inspected, persistent memory remained unchanged, and stdout/stderr were empty. Staged-diff secret scan is clean; the tracked repository has one existing synthetic token-shaped test fixture and no real credentials. One opt-in real provider summary/apply and commit/push remain pending; no quota-consuming request was sent.

### Sprint 985 - typed project references and live diagnostic retrieval

Extended the bounded project graph with explicit group membership, routing endpoints, teardrop anchors, diagnostic-to-object links, and serialized schematic-page membership. Live `project.context` carries bounded native DRC/ERC findings, tagged with engine, code, severity, and object ID; turn metadata counts diagnostic results that survived context packaging. Typed-project adaptation now preserves diagnostics attached beside the GUI's `typed_state` envelope. Incremental stale-edge removal and the GUI wrapper are covered by 24 focused Python contracts; Pyright reports zero diagnostics for the changed index. The Qt MinGW Release build succeeded and full CTest passed 113/113. The app-owned GUI-map run used an isolated project copy, clicked seven mapped controls, verified `ZERO_LENGTH_TRACK` on `T_SPRINT985_ZERO` and three DRC/ERC diagnostics in the actual context activity, then truthfully reported that the local profile has no provider key and sent no provider request. All nine screenshots, report, stdout, and stderr were inspected; the repository's stale source-contract anchor was updated and the contract passes under the Agent venv. Broader C3 graph/layer coverage, retrieval benchmarks, and provider-bound validation remain open.
# Phase 9 / Sprint 1006 — Model-aware context allocation

The active `sprint-991-hybrid-memory-retrieval` branch now budgets the assembled CCad project-context package from an explicitly refreshed provider model catalog. The allocation is capped at 25% of the model context window and 8,192 estimated tokens; unknown or invalid catalog limits use the configured bounded character fallback. Catalog refresh is explicit and process-local, and does not add a hidden provider request to ordinary chat turns.

Verification passed on 2026-09-26: Qt/MinGW Release build and full CTest, 117/117 passed. Workspace-only evidence manifest: `artifacts/evidence/sprint1006-model-context-allocation-r1.json` (SHA-256 `6754001F7D912E91F4E0F976EA78565A52B13CC17CB81CEEA1DE4C6BC4E6BC15`). Exact full-request tokenization remains open because the Gemini count endpoint is an additional provider request; opt-in real-provider/Langfuse receipt and hosted CI/PR remain separately unverified.
