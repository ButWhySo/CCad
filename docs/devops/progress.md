...
- Sprint 1010 was locally verified and pushed to `main` at `acd2ab5071f35c520f84ebc1d92bce4c6f4d8c48`. Hosted Python CI and evidence-manifest validation pass; core Linux, GUI Linux, and Windows CTest fail specifically on `agent_project_index`. Anonymous GitHub check annotations reveal the failing test name but no assertion output, and job-log download requires authentication. Sprint 1011 adds verbose reruns of failed CTests and publishes their output to job summaries/artifacts/annotations. The diagnostic workflow contract passes and the Qt/MinGW Release build/full CTest pass 119/119; workspace-only manifest `artifacts/evidence/sprint1011-ctest-failure-diagnostics.json` (SHA-256 `7DFDB251C5E45B953C224F5DAD82FD7797BD3BB0E6A5B04F8A4B11B45F7CCDD6`). The root cause and green hosted status remain open; do not mark hosted CI complete. CD remains unconfigured with no deployment target.
- Sprint: 1010
