...
- Sprint 1010 was locally verified and pushed to `main` at `acd2ab5071f35c520f84ebc1d92bce4c6f4d8c48`. Hosted CTest failed on `agent_project_index` in all three native jobs. Sprint 1011 verbose diagnostics showed both `test_project_index.py` and `test_agent_context_contract.py` depended on ignored `artifacts/demos/sprint160-placement-crash-ci-final.ccad.json`, absent in clean hosted checkouts. The tests now use deterministic typed fixtures, and a CI contract blocks regression. The two changed tests and CI contract pass; the official Qt/MinGW Release build and full CTest pass 119/119. Workspace-only manifest: `artifacts/evidence/sprint1011-ctest-fixture-fix.json` (SHA-256 `376998C90A02A27397DDFB7CEBDAD6CB5FD0AC331D8CFEB44C2908B791CE403B`). Hosted validation on the post-fix SHA remains pending. Diagnostics-only workflow manifest: `artifacts/evidence/sprint1011-ctest-failure-diagnostics.json` (SHA-256 `7DFDB251C5E45B953C224F5DAD82FD7797BD3BB0E6A5B04F8A4B11B45F7CCDD6`). CD remains unconfigured with no deployment target.
- Sprint: 1010
