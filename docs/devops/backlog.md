...
- [ ] Sprint 1011: hosted core Linux, GUI Linux, and Windows CTest failed `agent_project_index`; verbose output identified two geometry tests reading an ignored, untracked board artifact. Both tests now use deterministic typed fixtures, with a CI regression guard. Both changed tests pass, and Qt MinGW Release/full CTest pass 119/119; exact-SHA green hosted run remains open.
