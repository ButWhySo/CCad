...
## CI / CTest / CD health snapshot (Sprint 1020)

- [x] Current `main` CI run `36257220845` on `3587c6e4b50d5a899abb54aa08593b0c5baeb5f1` passes all five configured jobs; CTest runs in `core-linux`, `gui-linux`, and `core-windows`. The earlier Python dependency/contract, Linux `-Werror`, and ignored-fixture failures were repaired in Sprints 1010 and 1011; later runs, including this exact HEAD, pass.
...
- [x] Sprint 1015 follow-up: replace the local verifier's stale fixed count of 115 CTest cases with validation of the complete zero-failure summary and positive discovered count; Qt/MinGW Release and full CTest pass 120/120, hosted CI on exact merged `main` SHA `ce8a518906baaef5b88e278aaa12675c3e6a447f` is green in run #547, and no CD target is configured. Manifests: `artifacts/evidence/sprint-1015-ci-ct-pipeline-triage.json` (SHA-256 `B2D42AA4CC2B83F6306BC84A73BBB7F9DC8D6AA165B4DB6B617FA7A292442084`) and `artifacts/evidence/sprint-1015-ci-ct-hosted-closure-r1.json` (SHA-256 `17A22EB1EA936A12A030B2901E441292650A5B2BB8AA59642D72D8967AAC8479`).
- [ ] Verifier follow-up: replace timestamp-only reuse freshness checks with source-content fingerprints so Git checkout timestamp refreshes do not force redundant full builds; never reuse when source fingerprint differs or is unavailable.
- [x] Sprint 1011: hosted CI run `36235647915` passes all jobs on `4698996db768bd01f746a2a874edb07cff44a3c4`; verbose diagnostics identified the ignored local board-fixture dependency behind `agent_project_index`, and both tests now use deterministic typed fixtures. Qt MinGW Release/full CTest pass 119/119.
- [x] Sprint 1010 CI/CTest repair: stale provider/checkpoint contracts updated, Python dependencies installed in native CTest jobs, Linux `-Werror` credential-helper regression fixed, and per-job build/test diagnostics retained. Full Python lane, Qt Release, local CTest 119/119, and hosted all-platform jobs pass on the merged follow-up SHA. No CD deployment workflow/target is configured.
- [ ] Sprint 1009 is locally implemented and fully tested, awaiting publication: per-record safe labels distinguish automatic retrieval, actual Memory Summary inclusion, and explicit deep retrieval; package output uses hashed memory keys and exact budget-surviving channel counts, including in `memory.retrieve`/package observations. Qt/MinGW Release build and full CTest pass 119/119. Pyright has two pre-existing orchestrator import-symbol diagnostics. Manifest `artifacts/evidence/sprint1009-memory-exposure-provenance-r3.json` (SHA-256 `97B97406372ACE0BB26DE6DA669C0BD668C4D19429C707F0E2B34FF9B012520B`). Staged-addition credential scan and whitespace check are clean. Commit, local Git merge, push, and remote SHA verification remain pending.
- [x] Sprint 1008: bounded initial Memory Summary now carries source-backed stable LTM/episodic preferences and corrections plus explicitly high-importance facts; STM, low-priority facts, and secret-bearing entries are excluded. Provider-package, context, and memory contracts pass; changed-module Pyright is clean; Qt/MinGW Release and full CTest pass 119/119. The CRLF-sensitive PowerShell-source policy test now normalizes line endings. Workspace-only manifest `artifacts/evidence/sprint1008-memory-summary-r2.json` (SHA-256 `CEAC8CE780EC8DFDBB7E5B09FA494ECB29D4B07CC1DEAC98536C8D3D5A12433D`). Feature commit `a1603e0` merged as `5d6aab6`; pushed main SHA `5d6aab66871cf93e389c215920bfc7ee33a5f099` verified.
