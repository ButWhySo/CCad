...
- [ ] Sprint 1011: hosted core Linux, GUI Linux, and Windows CTest all report `agent_project_index` failure on Sprint 1010 SHA `acd2ab5071f35c520f84ebc1d92bce4c6f4d8c48`. Anonymous GitHub annotations omit assertion details and job logs require authentication; workflow now reruns the failed case verbosely and publishes its output. Local Qt/MinGW Release/full CTest pass 119/119; root-cause fix and exact-SHA green hosted run remain open.
- [ ] Sprint 1010 CI/CTest repair: stale provider/checkpoint contracts updated, Python dependencies installed in native CTest jobs, Linux `-Werror` credential-helper regression fixed, and per-job build/test diagnostics retained. Full Python lane, Qt Release, and local CTest 119/119 pass; hosted `agent_project_index` failures are tracked in Sprint 1011. No CD deployment workflow/target is configured.
