...
For supported rectangular holes, `buildRectangularThermalSpokes()` also omits axial segments whose interior overlaps a hole rectangle. This remains geometry preparation only; non-rectangular clipping and refill integration are deferred.
The hole blocker uses strict interior overlap for horizontal/vertical rays, preserving conservative boundary behavior while avoiding bridges through rectangular hole interiors. General segment/edge intersection remains a future geometry slice.
The blocker now tests each spoke against every edge of any valid hole polygon, plus endpoint containment. This supports triangular and other polygon holes without changing the preparatory API; fill-result/refill integration remains deferred.
`calculateZoneFill(zone, pads, gap, spoke_width)` augments a successful fill result with spokes for non-empty matching-net pads using their first positive copper shape dimension. The CLI calls this with explicit 0.5 mm defaults and emits `thermal_spoke_count`; persistence/export still carry contours only.
Board-aware spoke generation filters explicit zone/pad copper-layer overlap and suppresses `direct`, `solid`, and `none` connection modes; empty layer sets retain compatibility fallback. KiCad reference checked; per-pad connection overrides and PTH-only mode remain deferred.
...
Sprint 526 migrates `pcb add-target` loading to the shared context; compatibility writing remains for audit continuity.

Sprint 527 migrates `pcb add-pad` loading to the shared context; compatibility writing remains for audit continuity.

Sprint 528 migrates `pcb set-pad` loading to the shared context; compatibility writing remains for audit continuity.
...
Sprint 535 migrates `pcb apply-route-segment` loading to the shared context; compatibility writing remains for audit continuity.

Sprint 536 migrates `pcb apply-route-polyline` loading to the shared context; compatibility writing remains for audit continuity.
...
`src/ccad_gui/agent_settings_dialog.cpp` owns provider setup UX. Test Provider sends `agent.test_provider`, a transient provider/model/key probe; Save sends persistent `agent.set_config`. Keys remain process-memory only. `AgentPanel::handlePythonError` surfaces bounded redacted stderr diagnostics. Provider-state model names feed `provider_model_` and the visible model chip.
...
### Sprint 725 agent loop quota boundary

`src/ccad_agent/orchestrator.py::invoke_agent_run()` passes bounded LangGraph
`recursion_limit` per run. `CCAD_AGENT_RECURSION_LIMIT` tunes it, clamped to
...
`src/ccad_agent/orchestrator.py::init_provider` branches strictly on configured
provider. Unrelated `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`, or `OPENAI_API_KEY`
...
password-reveal path. `tests/physical_ui_robot.py` selects providers/models by
exact IDs; do not infer provider rows from older screenshots.
...
The index now parses explicit two-corner `bounding box`/`rectangle` requests and queries the bounded grid for intersecting AABBs. Spatially selected entities pull in only live DRC/ERC records that reference those entity IDs; diagnostics elsewhere remain excluded. BM25 text includes typed descriptions and diagnostic engine/code/severity/message, with the existing secret-shaped-value rejection. Regression contracts cover every currently serialized PCB collection's layer fields, stale geometry and diagnostics, exact cross-domain net/component and sheet relationships, context-package inclusion, and bounding-box intersection behavior. Net equality remains an ID association, not proof of physical continuity. CCad still has no individual design-rule identity objects; unconnected library pins, durable sheet paths, artifact/proposal/functional-block graph links, and transaction-delta updates remain open.

The app-owned validation is `sprint986-project-spatial-index-*` in `scripts/run_ui_map_mouse_target_demo.ps1` and `src/ccad_gui/main.cpp`. It adds an isolated real zero-length track, runs authoritative DRC from the mapped Agent panel, and submits a region query with the provider disabled. It retains only the before, DRC-ready, and bounded-result screenshots while machine-logging each mapped interaction. See Sprint 986 in `docs/devops/sprints/agent-production-todo.md` for exact test/build/visual evidence.

### Sprint 980 typed project retrieval foundation

`src/ccad_agent/project_index.py` derives a bounded per-entity index directly from the current `typed_state.project` snapshot. It supports exact IDs/references/layers, BM25 lexical ranking, one-hop component/layer/shared-net association, and an mm-coordinate spatial hash over supported object bounds. Each retrieval synchronizes source object hashes against the live typed snapshot, updates only changed index entries, and returns the source revision and per-result retrieval provenance. `same_net` is explicitly only shared net assignment, not proof of physical copper continuity; spatial distances are axis-aligned-bounds distances, not copper-clearance calculations. The context broker retains only the active project's index so it does not retain another project's design in the process cache, and the context package carries concise relevant entities even when the complete snapshot is omitted for size. The Agent panel's `context_state` line reports the actual number of retrieved project matches. `scripts/test_project_index.py` covers the index, incremental broker integration, bounded context injection, and large-snapshot compaction. The Sprint 980 UI-map flow asks about `U_DEMO`, observes 10 project matches with the provider disabled, and confirms transcript persistence. Qt MinGW Release and CTest pass 113/113; all four distinct screenshots and stdout/stderr were inspected. Full C3 parity remains open for rule/diagnostic/sheet/artifact identity, broader graph edges, arbitrary bounding-box/DRC-marker search, and benchmark evidence.

### Sprint 982 multilayer PCB project context

`src/ccad_agent/project_index.py` treats `layer_id`, `start_layer_id`, `end_layer_id`, and declared `layers`/`layer_ids` as exact per-entity memberships. The index signature and incremental synchronization include the complete normalized layer-ID set, so editing a via endpoint drops its previous layer edge and adds the new one without rebuilding unchanged records. Exact layer retrieval can therefore find a through via from either endpoint; output retains both endpoint fields and the deduplicated `layer_ids` list. `context_package.py` carries only IDs on entities that survive package bounding and reports the resulting safe IDs/count in the per-turn context metadata. In `review_window.cpp`, both normal Add Via canvas clicks and the automation placement path derive through-via endpoints from the board's first and last copper layers; boards without two distinct copper layers fail before undo history or project mutation. This is not full project-graph coverage: all object variants, zones, rules and diagnostics still need explicit indexing and bounded query validation. Sprint 982 tests and visual proof are recorded in `docs/devops/sprints/agent-production-todo.md` and the current sprint status in `docs/devops/progress.md`.

### Sprint 984 exact native PCB-net retrieval

`ProjectIndex` derives bounded `board_net` records from `net_id` values on typed PCB objects and indexes each derived record in the same shared-net association graph. It preserves native IDs through `_project_retrieval_payload`; context-package metadata counts only board-net records that survive the size budget, `orchestrator.py` carries the count on `context_state`, and the Agent panel displays `PCB nets: N`. This is a typed ID association, never proof of geometric copper continuity. `_exact()` prioritizes a full structured colon-delimited identity over any embedded short alias, even in a natural-language query. The app-owned verification path is `sprint984-board-net-retrieval-*` in `scripts/run_ui_map_mouse_target_demo.ps1` and `src/ccad_gui/main.cpp`; diagnostic/sheet graph edges, arbitrary bbox/DRC-marker retrieval, and measured C3 benchmarks remain open.

### Sprint 983 production-shaped project index coverage

`src/ccad_agent/project_index.py` now reads real serialized pad copper layers from `padstack.layer_set`, as emitted by `src/ccad_core/serialize.cpp`, as well as top-level layer sets, explicit `layer`/`layer_id`, via endpoint spans, and route-request `preferred_layer_id`. Its typed board extraction covers footprints, pads, tracks/arcs, vias, zones, graphics, text, dimensions, keepouts, route requests, groups, targets, barcodes, reference images, tables, placement regions, and teardrops. Reference-image payload bytes remain excluded by the safe typed-state boundary; IDs, layer membership, and positions remain searchable. Geometry extraction now includes both footprint courtyards, axis-aligned keepout/placement-region Rects, and the serialized mm coordinates/sizes used by images and tables. The index also exposes CCad's scalar `board.design_rules` values as one searchable board-level value object; the C++ model has no per-rule identity objects, so no synthetic rule IDs are created. Rule values use their serialized units (for example, `_nm` fields remain integer nanometers) and survive `context_package.py` allowlisting and compaction. `scripts/test_project_index.py` covers nested production serialization, layer families, bounds, rule edits, incremental updates, and packaged rule values. The 20 focused contracts pass; changed runtime modules have zero Pyright diagnostics, Qt MinGW Release and full CTest pass 113/113. The provider-disabled GUI-map scenario passed seven mapped actions against an isolated serialized-pad-layer fixture; three feature-relevant screenshots and stdout/stderr were inspected. Remaining C3 work includes diagnostic-to-object relationships, stable rule/diagnostic/artifact identities where the kernel supplies them, sheet hierarchy, arbitrary bbox queries, DRC-marker spatial search, and measured retrieval benchmarks.

### Sprint 981 schematic net-member retrieval

`src/ccad_agent/project_index.py` now expands bounded `schematic_net.members` into separately addressable `schematic_pin` retrieval records keyed by source net, component ID, and pin name. Those records retain symbol ID/reference, net ID, and `membership_kind=schematic_net_member`, without fabricated pin coordinates. Net retrieval can include declared pins, symbols, wires, and labels; board pads/tracks/vias retain separate `same_net` association labels. Extraction builds a bounded per-net lookup once instead of rescanning every sheet net for each indexed net, and incremental signatures include graph fields so changed membership replaces stale pins/edges without reindexing unchanged entities. The context package counts only pin/symbol records that survive its output budget, and the Agent activity line reports these real packaged counts. Schematic membership is explicitly a netlist assignment, not geometric copper continuity; unconnected library pins, full hierarchy, and broader graph coverage remain open. `scripts/test_project_index.py` passes 13/13; Pyright is clean; Qt MinGW Release and CTest pass 113/113. The provider-disabled `sprint981-schematic-project-graph` UI-map flow completed seven actions, visibly showed 10 results (3 pins, 1 symbol), preserved transcript behavior after `/clear`, and had four inspected screenshots and reviewed stdout/stderr. clangd reports no source diagnostics; its optional ExtractFunction action probe in `agent_panel.cpp` emits internal break/continue errors.

### Sprint 1009 memory exposure provenance

`context_broker.py` attaches allowlisted exposure channels to safe retrieval metadata: automatic selection, actual inclusion in the deterministic Memory Summary, and explicit deep search. Opaque internal IDs are carried only long enough for `context_package.py` to associate final included entries with provenance; provider/context metadata and Langfuse observations export a SHA-256-derived 16-character memory key and channel labels, never raw IDs or content. Package channel counts are recomputed from entries and summary text that survive truncation. `orchestrator.py` returns per-record exposure channels and counts with the existing local memory-search result and records safe per-memory manifests/counts on `memory.retrieve` and context-package observations. Contracts cover `scripts/test_context_broker.py`, `scripts/test_context_budget.py`, `scripts/test_agent_memory_search_tool.py`, and the source contract. Qt/MinGW preflight and Release build pass; full CTest passes 119/119. Pyright has two pre-existing orchestrator import-symbol diagnostics. Workspace-only manifest `artifacts/evidence/sprint1009-memory-exposure-provenance-r3.json` (SHA-256 `97B97406372ACE0BB26DE6DA669C0BD668C4D19429C707F0E2B34FF9B012520B`).

### Sprint 977 context broker and runtime memory retrieval

`src/ccad_agent/context_broker.py` derives bounded task/editor/selection/entity/history signals, applies the existing `MemoryManager` tier/namespace filters before lexical ranking, caps selected records by estimated memory tokens, and caches the result against thread, project identity/revision, request signals, history count, and memory generation. A thread's cached context is invalidated when its memory changes, a task starts/ends, or the thread changes; project identity is also a cache-generation input. The `TurnContext` carries only safe counts/digests for diagnosis; the provider-facing context package separately carries the authorized memory entries, a title-only bounded summary, and an allowlisted content-free scope/count manifest.
...
# Sprint 987 schematic metadata retrieval

`src/ccad_agent/project_index.py` indexes CCad's serialized schematic `components[].fields[]` as bounded scalar properties, preserving `name`, `text`, and boolean visibility and filtering sensitive property labels. `sheets[].name` and a normalized project-relative `file_path` are exact/lexical searchable aliases; absolute paths are excluded. `context_package.py` independently allowlists at most 32 properties per entity, bounds values, drops sensitive names/values, and revalidates paths before provider context inclusion. Agent chat IPC extracts the native project ID from `typed_state.project.id` and passes it as `project_id`, keeping durable memory and project retrieval scoped to the actual project. The official harness scenario `sprint987-schematic-metadata-final` checks live typed serialization, per-turn retrieval, no provider call, seven mapped actions, transcript retention after `/clear`, and three inspected visual checkpoints. Thirty-two project-index/context tests and changed-module Pyright pass; Qt Release and CTest pass 113/113. Per-rule IDs, notes, annotations, artifact IDs, and functional-block summaries remain unavailable unless the native model gains authoritative records.
...
`src/ccad_agent/orchestrator.py` owns JSON-RPC dispatch and long-lived Agent state. `handle_human_message(req)` now owns a complete chat turn outside the protocol-loop scope, while the dispatcher delegates to it; terminal paths return from the handler rather than altering dispatcher control flow. Provider model-list HTTP requests and response shaping live in `src/ccad_agent/model_catalog.py`; orchestrator facade methods retain the existing IPC API and pass the canonical safe failure classifier. Provider catalog calls remain explicit refresh operations, use bounded timeouts, and are covered by controlled-response parser tests.

# Sprint 989 bounded BM25 memory and project-history retrieval

`src/ccad_agent/lexical_retrieval.py` implements deterministic BM25 for already-bounded, scope-filtered local documents and reports score plus matched terms. `MemoryManager.retrieve_with_metadata()` applies it to enabled and loaded task, conversation/project, and user-global tiers after expiry pruning; final inclusion still passes through `ContextBroker`'s entry/character/estimated-token limits. `ConversationStore.search_turn_records()` uses the same scorer over at most 256 lexical candidates. `search_project_history()` first filters compact recaps to the exact project ID in SQLite (up to 500), ranks summaries, expands at most 16 threads, and loads only their referenced TurnRecords whose `project_id` still matches. It excludes the active thread and requires two matched terms for multi-term queries. `handle_human_message()` merges at most four such results into the current-thread retrieval, with a total prior-turn cap of eight. `context_package.py` preserves source thread, turn, and message IDs in the provider envelope. Missing project identity yields no cross-thread history. Embeddings, RRF/MMR, importance/recency weighting, preference-correction weighting, and provider tokenizer budgets remain open; BM25 score must not be described as semantic similarity.

Focused contracts: `scripts/test_lexical_retrieval.py`, `scripts/test_memory_manager.py`, `scripts/test_conversation_store.py`, `scripts/test_context_budget.py`, `scripts/test_context_broker.py`, `scripts/test_conversation_runtime.py`, `scripts/test_agent_memory_search_tool.py`, and `scripts/test_agent_context_package.py`.
...
# Current delivery note — Sprint 1010

The CI workflow's native CTest jobs run Python-backed contracts, so each now provisions Python 3.12 and `src/ccad_agent/requirements.txt` before configure/build/test. Linux GUI builds use warnings-as-errors; Windows-only credential helpers must remain platform-guarded. CI stores configure/build/CTest logs per job and surfaces Linux compiler errors in annotations. Sprint 1010's pushed CI run also failed `agent_project_index` on Linux and Windows; Sprint 1011 adds verbose failed-test reruns because anonymous GitHub logs conceal the assertion. Do not mark the hosted lane fixed until that root cause is addressed and the exact SHA is green. See `docs/devops/sprints/agent-production-todo.md` and `docs/devops/progress.md` for current verification status. There is no configured CD/deployment workflow or target in this repository.
