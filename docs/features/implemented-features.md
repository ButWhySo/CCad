...
`project.context` now carries bounded results from the native DRC and ERC engines, preserving each diagnostic's engine, code, severity, message, and affected object ID. The Agent project index can retrieve those records by diagnostic code or object identity and follow explicit typed references for group members, route-request endpoints, teardrop anchors, diagnostic targets, and nested schematic page membership. Page membership is assigned only to serialized schematic documents, never to the project container. Incremental edits refresh and remove stale graph edges. Results remain bounded and diagnostic counts in Agent activity reflect the project entities that survived context packaging. Focused contracts include object-linked diagnostics, explicit-reference traversal, hierarchy, and stale-edge removal. This is a bounded C3 slice; complete object/layer coverage and retrieval benchmarking remain open.
...
Before model reasoning, the Python orchestrator extracts bounded design/task signals and builds a thread/revision-scoped `TurnContext`. Enabled memory tiers are scope-filtered and ranked deterministically; selected records are limited by a configurable estimated-token budget (`CCAD_AGENT_MEMORY_TOKENS`, default 1000) and eight-entry ceiling. The provider envelope includes those authorized records, a bounded title-only Memory Summary, and an allowlisted availability/count manifest. `ccad_search_memory` is a real read-only local LangChain tool that lets the model request additional relevant records during the same turn, preserves prior valid matches, re-applies the budget, and reports an incremented context version. Diagnostics expose counts and digests, not prompt or memory text. Project-scoped memory/indexing, semantic retrieval, provider-tokenizer accounting, and a unified Langfuse root trace are not implemented by this slice. Offline contracts: `scripts/test_context_broker.py`, `scripts/test_context_budget.py`, `scripts/test_agent_context_contract.py`, `scripts/test_agent_native_tool_catalog_protocol.py`, and `scripts/test_agent_memory_search_tool.py`.
