...
Progress counter: Phase 7 / 7, Sprint 182 complete on `sprint-182-ui-map-index-cache`.
...
- Native GUI can keep a local live UI-map socket open while the GUI runs, serving repeated JSON Lines `ui.map`, `ui.map_compact`, `ui.role_summary`, dirty-node `ui.map_delta`, `ui.wait_for_delta`, `ui.watch_delta`, indexed `ui.index_stats`, `ui.get_node`, `ui.nodes_by_role`, `ui.find`, `ui.hit_test`, `ui.target`, `ui.target_board_point`, indexed `ui.nearest_canvas_object`, `ui.canvas_click`, `ui.canvas_drag`, `ui.current_tool`, `ui.cancel_tool`, high-level PCB workflow methods, `ui.trigger_safe`, `ui.epoch`, active-layer, and active-net requests for agents.
