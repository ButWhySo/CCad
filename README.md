...
- `agent.methods` merges callable native GUI methods with the authoritative `ccad help --format json` command inventory. CLI entries include their schema, usage, policy-derived side effect, and result shape, and are marked non-callable in the GUI provider until guarded CLI execution is connected.

Sprint 963 verification passed the Qt/MinGW Release build, full CTest (92/92), offline native-catalog protocol test, and live Qt UI-map check. The registry exposed 59 callable GUI methods and 111 non-callable CLI descriptors; the inspected screenshots and empty stdout/stderr logs are local-only under `artifacts/screenshots/sprint963-registry-20260923/`.
...
- Agent Settings persists MCP server rows through `mcp_servers`; the read-only `agent.mcp_status` route reports normalized entries and explicitly reports `runtime:"not_started"` and `process_execution:false`. Saving a row does not launch or connect a server.
