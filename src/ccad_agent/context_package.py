...
def format_large_context_explanation(report: dict,
    ...
    lifecycle_detail = (
        ...
        "provider execution. The runtime loads only enabled namespaces (up to 64 cached "
        "entries per tier), removes expired entries during load and retrieval, then ranks matches by query-term overlap, "
        ...
        "not a retrieval filter. STM is transient and session-scoped; LTM is durable and "
        ...
        "no automatic learning from ordinary chat. Normalized exact duplicates are "
        "deduplicated, durable namespaces retain at most 64 newest records, secret-bearing "
